    ...
    static List<Integer> dynamicArray(int n, List<List<Integer>> queries) {
        ...
        for(List<Integer> query : queries){
            ...
            if(query.get(0)==1){
                ...
                lastAnswer = seqList.get(seqIndex).get(query.get(2)%seqList.get(seqIndex).size());                result.add(lastAnswer);
            }
        }
        return result;
    }
...
Query: 2 x y
Find the sequence, seq , at index ((x^lastAnswer%N) in seqList.
Find the value of element y%size in  seq (where  is the size of ) and assign it to lastAnswer.
